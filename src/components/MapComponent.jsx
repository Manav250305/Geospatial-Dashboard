...
import { MapContainer, TileLayer, useMapEvents, Marker, Popup, Circle, Polyline, GeoJSON } from 'react-leaflet';
    ...
    <>
      {data.map((point, index) => (
        <Circle
          ...
          radius={200}
          fillColor={
            point.intensity > 0.7 ? '#ff4444' : 
            point.intensity > 0.5 ? '#ffaa44' : 
            '#44ff44'
          }
          color={
            point.intensity > 0.7 ? '#ff0000' : 
            point.intensity > 0.5 ? '#ff8800' : 
            '#00ff00'
          }
          fillOpacity={0.6}
          opacity={0.8}
        >
          <Popup>
            <div>
              <strong>Heat Intensity: {(point.intensity * 100).toFixed(1)}%</strong><br/>
...
const Legend = ({ mapData }) => {
  ...
  if (mapData.type === 'uhi') {
    return (
      <div className="absolute bottom-4 right-4 bg-white p-3 rounded shadow-lg z-[1000]">
        <h4 className="font-bold text-sm mb-2">Heat Intensity</h4>
  ...
  if (mapData.type === 'geojson') {
    return (
      <div className="absolute bottom-4 right-4 bg-white p-3 rounded shadow-lg z-[1000]">
        <h4 className="font-bold text-sm mb-2">Pothole Severity</h4>
        <div className="space-y-1 text-xs">
          <div className="flex items-center">
...
const MapComponent = ({ 
  ...
  const getGeoJSONStyle = (feature) => {
    ...
    return {
      color: severity === 'severe' ? '#ff0000' : 
             severity === 'moderate' ? '#ffaa00' : '#00ff00',
      radius: severity === 'severe' ? 8 : 
    ...
    layer.bindPopup(`
      <div>
        ...
        <strong>Severity:</strong> <span style="color: ${
          severity === 'severe' ? '#ff0000' : 
          severity === 'moderate' ? '#ffaa00' : '#00aa00'
        }">${severity}</span><br/>
  ...
  return (
    <div className="flex-1 relative">
        ...
        {regionCenter && (
          <>
            <Marker position={regionCenter}>
              <Popup>
                <div>
                  <strong>Selected Center Point</strong><br/>
                  Lat: {regionCenter.lat.toFixed(4)}<br/>
      ...
      {/* Map Instructions */}
      <div className="absolute top-4 left-4 bg-white p-3 rounded shadow-lg z-[1000] max-w-xs">
        <h4 className="font-bold text-sm mb-2">Map Instructions</h4>
